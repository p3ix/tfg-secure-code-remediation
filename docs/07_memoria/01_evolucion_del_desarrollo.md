...
## 14. Capa de explicaciones con IA (local primero)

Con el núcleo estable, abordé la capa de IA que da sentido al término "asistida por IA" del título, siguiendo el diseño fijado en [ADR-003](../02_decisions/ADR-003-ai-explanations-design.md). La decisión de partida fue **IA local primero**: como la herramienta analiza código de terceros, enviar fragmentos a una API externa sería incoherente con un trabajo de seguridad; por eso el proveedor por defecto es un modelo local servido por Ollama, con una API externa opcional reservada solo para comparativas. Para no comprometer la reproducibilidad ni la demostrabilidad, definí además un proveedor *stub* determinista que permite que toda la suite y el CI funcionen sin red ni modelo.

Implementé la capa en iteraciones pequeñas y verificables. Primero (IA-1) el **contrato**: una dataclass `AIExplanation`, una interfaz `AIProvider` degradable (devuelve `None` si no puede generar, para no tumbar el escaneo) y el `StubProvider`, junto con las variables `TFG_AI_*` de configuración. Después (IA-2) una **caché** de explicaciones por categoría/estándar/regla —no por fichero/línea, de modo que hallazgos equivalentes comparten explicación y el resultado es reproducible— y una **factoría** `get_ai_provider()` que respeta el kill-switch `TFG_AI_EXPLANATIONS_ENABLED`. En IA-3 cablé el campo opcional `ai_explanation` en la salida presentable y subí el contrato a `schema_version` 1.1 (campo ausente = sin explicación, sin romper consumidores del 1.0). En IA-4 añadí el bloque visual en el dashboard, siempre con un *disclaimer* de que el texto lo genera una IA y que la verificación la aportan Bandit/Semgrep. En IA-5 implementé el `OllamaProvider` real usando solo la librería estándar (`urllib`), con *timeout*, envío del *snippet* solo si se activa explícitamente, y degradación limpia ante errores de red o parseo; los tests mockean el HTTP para mantener la suite offline. En IA-6 endurecí el *prompt* frente a *prompt injection* (el contenido de terceros viaja entre marcadores y se instruye al modelo a no obedecer instrucciones embebidas) y añadí trazabilidad con `prompt_version` y un hash del *prompt* enviado, alineado con NIST SP 800-218A y OWASP Top 10 for LLM. En IA-7 definí una **evaluación cualitativa** con rúbrica reproducible (correctitud técnica, utilidad pedagógica y ausencia de alucinación) sobre el corpus MVP, comparando la línea base *stub* con el modelo local, documentada en [`docs/04_delivery/ai-explanations-evaluation-table.md`](../04_delivery/ai-explanations-evaluation-table.md). Finalmente, en una iteración posterior amplié la capa para que la IA, además del texto, devuelva un **ejemplo ilustrativo "antes/después"** del patrón (`example_before`/`example_after`): esto subió el contrato a `schema_version` 1.2 y el *prompt* a v4, revisó el ADR-003 (se permite mostrar un ejemplo educativo, nunca un parche aplicado ni verificado) y añadió su render visual en la web para todos los hallazgos.

El principio transversal de toda la capa es que la IA es **aditiva y opcional**: desactivada por defecto, degradable ante cualquier fallo y sin capacidad de modificar código (las remediaciones verificadas siguen siendo deterministas; el ejemplo "antes/después" es material educativo, no un parche aplicado). Así el sistema conserva su garantía principal —que un hallazgo y su corrección son comprobables por herramientas— y la IA aporta solo la capa explicativa.
...
- Evaluación de la capa IA: [`docs/04_delivery/ai-explanations-evaluation-table.md`](../04_delivery/ai-explanations-evaluation-table.md)

...
## Ampliación — Fase “proyectos reales” (materializada en el repositorio)
Además del corpus interno, el sistema ya puede analizar **código real** suministrado como:
...
- **Repositorio Git HTTPS** clonado de forma superficial (`POST /analysis/git-clone` y modo equivalente en `/dashboard`), con hosts permitidos por configuración (`TFG_GIT_ALLOWED_HOSTS`), timeout y **solo en entornos de confianza** (el clonado puede desactivarse con `TFG_ENABLE_GIT_CLONE=0`).

La consola web en `/dashboard` integra los tres flujos reales (ZIP, Git, ruta local) además de modos MVP/demo sobre `fixtures/mvp`. Manual de usuario: [`docs/04_delivery/manual-usuario-web.md`](04_delivery/manual-usuario-web.md). Hito de cierre de fase sin IA: [`docs/04_delivery/release-web-operativa-sin-ia.md`](04_delivery/release-web-operativa-sin-ia.md).

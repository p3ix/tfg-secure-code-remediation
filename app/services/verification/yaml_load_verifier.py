...
def verify_yaml_load_remediation(source_code: str) -> dict:
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        target_file = target_dir / "remediated_yaml_case.py"
        ...
        if not bandit_report.exists():
            raise RuntimeError("Bandit no generó report de verificación.")

        if not semgrep_report.exists():
            raise RuntimeError("Semgrep no generó report de verificación.")
        ...
        remaining_yaml_findings = [
            f for f in findings if f.mvp_category == "unsafe_yaml_load"
        ]

        return {
            "verification_kind": "unsafe_yaml_load",
            "applicable": True,
            "verified": len(remaining_yaml_findings) == 0,
            "reason": (
                "No quedan hallazgos de unsafe_yaml_load tras la remediación."
                if len(remaining_yaml_findings) == 0
                else "Siguen existiendo hallazgos de unsafe_yaml_load tras la remediación."
            ),
            ...
            "remaining_findings": [
                ...
                for f in remaining_yaml_findings
            ],
